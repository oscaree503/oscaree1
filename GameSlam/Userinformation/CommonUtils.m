...
+ (BOOL) isValidName:(NSString*)name {
    NSString *nameRegex = @"^[A-Z][a-z]*[\\s{L}\\s{M}\\s{N1}][A-Z][a-z]*$";
...
-(NSArray*)gettingwordArraySpaceSparatedfromString:(NSString*)stringInfo{

    NSInteger nwords = 20;
    NSArray* words;
    for (int i = 0; i <= 20; i++) {
    
        NSRange wordRange = NSMakeRange(i, nwords);
        words = [[stringInfo componentsSeparatedByString:@" "]subarrayWithRange:wordRange];
    }
        return words;
}
